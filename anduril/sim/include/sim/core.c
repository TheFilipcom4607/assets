// sim/core.c: the parts of the simulated machine that do not depend on which
// era of Anduril is running: the virtual clock, interrupt dispatch, the
// e-switch pin, the EEPROM, and the hand-off back to the browser.
//
// A variant (arch/sim.c for current releases, sim/legacy.c for Anduril 1)
// includes this file, defines SIM_F_CPU first, and supplies the two hooks
// declared in core.h.
...
// One ADC conversion, at whatever rate this MCU's ADC runs.
#define SIM_ADC_PERIOD_CYCLES  ((uint32_t)(SIM_F_CPU / SIM_ADC_HZ))
#define SIM_TICK_CYCLES        ((uint32_t)(SIM_F_CPU / 1000 * SIM_TICK_MS))
...
void sim_fire_pending(void) {
    ...
    while (sim_pend_pcint || sim_pend_adc || sim_pend_wdt) {
        ...
        else if (sim_pend_adc) { sim_pend_adc = 0; sim_woke = 1; sim_adc_deliver(); sim_adc_isr(); }
...
void sim_sleep_cpu(void) {
    ...
    if (! sim_woke) {
        ...
        if (! next) next = sim_now + (SIM_F_CPU / 4);
        sim_advance_to(next);
    }
    sim_in_sleep = 0;
    sim_io.sleeping = 0;
...
void sim_eeprom_write(uint16_t addr, uint8_t value) {
    ...
    sim_burn_cycles(SIM_F_CPU / 250);  // a real EEPROM byte write takes ~4 ms
}

__attribute__((export_name("sim_init")))
void sim_init(void) {
    sim_io.magic = SIM_MAGIC;
    sim_io.f_cpu = SIM_F_CPU;
